...
with (
    ...
    csv_write.writeheader()  # запишет заголовки
    all_data = []
    for i, dict_row in enumerate(csv_read):
        if i != 0: # исключая нулевую строку, делаем необходимые вычисления
            dict_row['id'] = i
            dict_row['age'] += 1
            all_data.append(dict_row)
    csv_write.writerows(all_data) # запишет все остальное
